...
###############################################################################################
# no labels for fitting
###############################################################################################
umap_nolabel = reducer.fit_transform(emb_df.values)
umap_nl_df = pd.DataFrame(umap_nolabel, index=emb_df.index).reset_index()
umap_nl_df['contig_id'] = [x.rsplit('_', 1)[0] for x in umap_nl_df['subcontig_id']]

recruited_df = umap_nl_df.merge(clust_df, on='contig_id')
recruited_df['type'] = 'recruited'
recruited_df['stage'] = ['anchor' if x in best_anchors
						 else 'not_recruited' for x in recruited_df['contig_id']
						 ]
...
recruit_list = list(recruited_df['contig_id'].unique())
unrecruited_df = umap_nl_df.query("contig_id not in @recruit_list")
unrecruited_df['type'] = 'not_recruited'
unrecruited_df['stage'] = 'not_recruited'
